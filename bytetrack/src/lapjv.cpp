	...
	int_t *free_rows, int_t *x, int_t *y, cost_t *v)
{
    ...
    //----------------------------------------
    // [rc] v[j]记录第j列最小值
    // [rc] y[j]记录第j列最小值所在的行数
	for (uint_t i = 0; i < n; i++) {
		x[i] = -1;
		v[i] = LARGE;                   // LARGE = 100_0000
		y[i] = 0;
	}
	for (uint_t i = 0; i < n; i++) {
    ...
    //----------------------------------------
    // [rc] 记录每一行原本求的每一列最小元素的下标
    // [rc] 如果一行有多个下表, 则把unique置为FALSE, 并把y[j]置为-1
	{
		int_t j = n;
		do {
			j--;
...
//  #define NEW(x, t, n) if ((x = (t *)malloc(sizeof(t) * (n))) == 0) { return -1; }
	...
	NEW(free_rows, int_t, n);                                   // [rc] 分配一个长度为n的int_t 型的数组
	NEW(v, cost_t, n);                                          // [rc] 分配一个长度为n的cost_t(double)型的数组
